...
#### JA HORRIBLE HACK override start-notebook.sh to fix in compatibility with RHOAI and jupyter start-notebook.py
#  RHOAI does not use single quotes around the tornado setting in the NOTEBOOK_ARGS value.  opehacks contains a shim
#  script that tries to fix this before execing the start-notebook.py

COPY opehacks /usr/local/opehacks
CMD  ["/bin/bash", "-c", "cd /home/joyvan; if [[ -a /usr/local/opehacks/ope-start-notebook.shim ]]; then exec /usr/local/opehacks/ope-start-notebook.shim; else exec start-notebook.py; fi" ]
