__global__ void reduce2(float *d_ivec, float *d_ovec, unsigned int n) {
  ...
  // Like reduce 1 but we use a dense set of threads to do the work to try
  // and reduce thread divergence
...
void reduce2bytes(uint64_t *readbytes, uint64_t *writtenbytes, int n, int blksize)
{ 
  uint64_t rb = 0, wb = 0;
