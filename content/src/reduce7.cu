__global__ void reduce7(float *d_ivec, float *d_ovec, unsigned int n) {
  extern __shared__ float sdata[];
  unsigned int tid = threadIdx.x;
  // each thread is responsible for positions in 16 blocks
  // eg. blk0.thread0: blk0[0] and blk1[0],
  //     blk1.thread0: blk2[0] and blk3[0]
  //     etc.
  unsigned int i   = blockIdx.x *
  	             (blockDim.x*16)   // note * 16
		     + threadIdx.x;
...
void reduce7bytes(uint64_t *readbytes, uint64_t *writtenbytes, int n, int blksize)
{ 
  uint64_t rb = 0, wb = 0;
  for (int len=n; len>1; len=len/blksize) {
    rb += len;                   // reads all len elements 
    if (n/blksize <= 1) wb += 1; // one for one block or partial block
    else  wb += len/(16*blksize); // one for every eight blocks
    len = len / 16;               // each step takes care of 16 blocks worth
  }
